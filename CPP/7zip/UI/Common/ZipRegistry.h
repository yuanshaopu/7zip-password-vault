...
namespace NExtract
{
  struct CInfo
  {
    ...
    CBoolPair SplitDest;
    CBoolPair ElimDup;
    CBoolPair TryPasswordVault;
    CBoolPair DeleteAfterExtract;
...
namespace NVault
{
  void Save_PasswordVault(const UStringVector &passwords); // Save: serialize -> DPAPI encrypt -> REG_BINARY
  void Read_PasswordVault(UStringVector &passwords);       // Read: REG_BINARY -> decrypt -> deserialize
}
#endif
