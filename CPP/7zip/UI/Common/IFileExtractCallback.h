...
*/
  ...
  virtual void SetPasswordDefined(bool defined) px \
  virtual void SetTryMode(bool tryMode) px \
  virtual void ResetErrors() px \
  virtual bool Get_PasswordIsDefined() const px \
  virtual bool Get_IsOK() const px \
  virtual bool Get_TryIsOK() const px \
  virtual UInt32 Get_NumBatchErrors() const px
#else
  #define Z7_IFACEM_IExtractCallbackUI_Crypto(px) \
