...
static LPCTSTR const kTryPasswordVault = TEXT("TryPasswordVault");
...
void CInfo::Save() const
{
  ...
  Key_Set_BoolPair(key, kTryPasswordVault, TryPasswordVault);
  Key_Set_BoolPair(key, kDeleteAfterExtract, DeleteAfterExtract);
...
void CInfo::Load()
{
  ...
  Key_Get_BoolPair(key, kTryPasswordVault, TryPasswordVault);
  Key_Get_BoolPair(key, kDeleteAfterExtract, DeleteAfterExtract);
...
namespace NVault
{
    ...
    static LPCTSTR const kPasswords = TEXT("Passwords");

    static void Vault_Serialize(const UStringVector& v, CByteBuffer& buf)
    {
        size_t size = 0;
        FOR_VECTOR(i, v)
            size += v[i].Len() + 1;   // each password plus terminating 0
        size += 1;                  // double 0 terminator
        buf.Alloc(size * sizeof(wchar_t));
        wchar_t* p = (wchar_t*)(Byte*)buf;
        FOR_VECTOR(i, v)
        {
            const UString& s = v[i];
            memcpy(p, s.Ptr(), (s.Len() + 1) * sizeof(wchar_t));
            p += s.Len() + 1;
        }
        *p = 0;
    }

    static void Vault_Deserialize(const Byte* data, size_t size, UStringVector& v)
    {
        v.Clear();
        const wchar_t* p = (const wchar_t*)data;
        const wchar_t* end = (const wchar_t*)(data + size);
        while (p + 1 <= end && *p != 0)
        {
            v.Add(p);
            if (v.Size() >= 10)   // max 10 entries
                break;
            p += wcslen(p) + 1;
        }
    }

    void Save_PasswordVault(const UStringVector& passwords)
    {
        CS_LOCK
            CByteBuffer buf;
        Vault_Serialize(passwords, buf);

        DATA_BLOB in;
        in.pbData = (Byte*)buf;
        in.cbData = (DWORD)buf.Size();
        DATA_BLOB out;
        if (!CryptProtectData(&in, L"7-Zip Password Vault", NULL, NULL, NULL,
            CRYPTPROTECT_UI_FORBIDDEN, &out))
            return;

        CKey key;
        if (CreateMainKey(key, kKeyName) == ERROR_SUCCESS)
            key.SetValue(kPasswords, out.pbData, out.cbData);   // CKey::SetValue(void*, size) writes REG_BINARY

        SecureZeroMemory(out.pbData, out.cbData);
        LocalFree(out.pbData);
    }

    void Read_PasswordVault(UStringVector& passwords)
    {
        passwords.Clear();
        CS_LOCK
            CKey key;
        ...
        if (OpenMainKey(key, kKeyName) != ERROR_SUCCESS)
            return;
        if (key.QueryValue_Binary(kPasswords, buf) != ERROR_SUCCESS)
            return;
        ...
        Vault_Deserialize(out.pbData, out.cbData, passwords);
        SecureZeroMemory(out.pbData, out.cbData);
        LocalFree(out.pbData);
    }
