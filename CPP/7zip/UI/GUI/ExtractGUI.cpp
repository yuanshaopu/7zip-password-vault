...
HRESULT CThreadExtracting::ProcessVirt()
{
  ...
  */
  ...
  #ifndef Z7_SFX
  if (res == S_OK && ExtractCallbackSpec->Get_IsOK())
  {
    if (HashBundle)
    {
  ...
  */
  ...
  if (!options.TestMode)
  {
    ...
    if (showDialog)
    {
      ...
      dialog.TryPasswordVault = options.TryPasswordVault;
      dialog.DeleteAfterExtract = options.DeleteAfterExtract;
      ...
      options.TryPasswordVault = dialog.TryPasswordVault;
      options.DeleteAfterExtract = dialog.DeleteAfterExtract;
